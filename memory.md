...
### 2026-09-21 — Play Store packaging: AAB, tablet screenshots, package rename
**What:** README rewritten in Avi's voice with the Jacob Roberts credit (porch in Leucadia, San Diego County); PRIVACY.md moved to repo root (PR #3 merged); repo made PUBLIC. Android package changed to `com.nameplace.myapp` (Play Console expects it; PR #4 open). AAB built from `C:\bld2` after `expo prebuild` and signed with a new upload key (RSA 4096, alias `nameplace-upload`) kept OUTSIDE the repo in `C:\Users\avioh\nameplace-keys\` (back it up). Gradle release config still uses the debug keystore, so any rebuild must be re-signed with jarsigner (strip META-INF/*.SF/*.RSA first). 7" (1080x1920) and 10" (1440x2560) tablet screenshots captured from x86_64 emulators with seeded demo pins into `assets/google_store/` (untracked, ~13MB, not committed).
**Why:** Play upload needs the right package and a non-debug signature; tablet screenshot slots are required.
**Open:** merge PR #4; delete merged remote branches (classifier blocks); history still holds the old `.env` (Supabase URL + anon key, commit 1158c6e) and old README now that the repo is public, rewrite not pushed (force-push blocked, scratch rewrite is stale); rotate/delete the Supabase key; plain `git push` hangs because env `GITHUB_TOKEN` is invalid and `~/.gitconfig` insteadOf embeds an old PAT; store copy (gonzo short/long description) not yet saved to `release/store-listing.md`, and its "works offline" claim is wrong (map tiles need network).
